...
from brain_age_pred.other.convert_model import load_synthseg_pretrained_weights
...
def main() -> None:
    ...
    # Load SynthSeg pretrained weights if specified
    synthseg_weights_path = cfg.get("model.synthseg_weights_path")
    if synthseg_weights_path:
        logger.info("Loading SynthSeg pretrained weights...")
        model = load_synthseg_pretrained_weights(
            model, 
            synthseg_weights_path, 
            show_architecture=cfg.get("model.show_architecture", True),
            verbose=True
        )
    else:
        logger.info("No SynthSeg weights specified, using random initialization")
