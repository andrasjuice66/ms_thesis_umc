#!/usr/bin/env python
"""
Two-step training script for brain age prediction and segmentation.
First trains on segmentation, then fine-tunes on both tasks.
"""
...
from brain_age_pred.training.trainer_two_step import TwoStepTrainer
...
def main() -> None:
    ...
    if not experiment_name:
        experiment_name = f'two_step_{timestamp}'
    ...
    logger = setup_logger("brain-age-two-step", log_file=log_dir / "train.log")
    ...
    if use_wandb:
        ...
        wandb.init(
            project = cfg.get("wandb.project", "brain-age-two-step"),
    ...
    # 8. ─── trainer ──────────────────────────────────────────── #
    logger.info("Initializing TwoStepTrainer...")
    ...
    trainer = TwoStepTrainer(
        ...
        log_dir=log_dir,
        use_wandb=use_wandb,
        wandb_project=cfg.get("wandb.project", "brain-age-two-step"),
    ...
    # 9. ─── train ────────────────────────────────────────────── #
    logger.info("Starting two-step training...")
    try:
        t0 = time.time()
        
        # Check if a segmentation checkpoint is provided
        seg_checkpoint = cfg.get("model.seg_checkpoint", None)
        if seg_checkpoint:
        ...
        json.dump(best_dice_info, (ckpt_dir / "best_dice_info.json").open("w"), indent=2)
        ...
        if use_wandb:
            ...
            wandb.log({
                ...
                "best_val_dice": best_dice_info["value"],
                "best_val_dice_epoch": best_dice_info["epoch"] + 1
            })
