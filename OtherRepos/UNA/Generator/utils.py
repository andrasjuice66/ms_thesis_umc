...
def normalize_path(path):
    """Normalize path for consistent usage across the application."""
...
# When loading paths in prepare_paths method:
def prepare_paths(self, setup_dict):
    ...
    for key, value in setup_dict['paths'].items():
        if value:
            # Normalize root path
            root_path = normalize_path(setup_dict['root'])
            # Join and normalize the combined path
            self.paths[key] = normalize_path(os.path.join(root_path, value))

    # read names - also normalize this path
    split_file_path = normalize_path(os.path.join(setup_dict['root'], setup_dict[self.split]))
