...
batch_synthsr.py
-----------------------------------------------
Run SynthSR on all NIfTI volumes in a directory structure
and recreate the same directory structure with super-resolution outputs.
"""
...
def run_synthsr(in_img: Path, out_dir: Path, threads: int = 4, use_gpu: bool = False) -> Path:
    """
    Launch SynthSR through Docker using mackenzieasnyder/synthsr:latest.
    The function returns the path to the generated super-resolution image.
    """
    # Keep the same filename for the super-resolution output
    sr_path = out_dir / in_img.name
    ...
    print(f"[SynthSR] Processing {in_img} {'(GPU)' if use_gpu else '(CPU)'}")
    print("[SynthSR] " + " ".join(cmd))
    
    try:
        subprocess.check_call(cmd)
        print(f"[SynthSR] Successfully processed {in_img}")
        return sr_path
    except subprocess.CalledProcessError as e:
        print(f"[SynthSR] Error processing {in_img}: {e}")
        raise
...
def setup_wandb(project_name: str = "synthsr-batch-processing") -> None:
    """
    ...
    wandb.init(
        ...
        config={
            "tool": "SynthSR",
            "processing_type": "batch_super_resolution"
        }
    )
...
def main(input_dir, output_dir) -> None:
    ...
    use_gpu = True  # Enable GPU acceleration for Docker
    ...
    for i, nifti_file in enumerate(pbar, 1):
        ...
        try:
            ...
            # Run SynthSR
            sr_path = run_synthsr(nifti_file, file_output_dir, threads=threads, use_gpu=use_gpu)
...
if __name__ == "__main__":
    ...
    input_dir = Path("/mnt/c/Projects/thesis_project/Data/brain_age_preprocessed_no_csf/IXI")
    out_root = Path("/mnt/c/Projects/thesis_project/Data/brain_age_synthsr/IXI")
