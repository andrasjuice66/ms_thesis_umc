#!/usr/bin/env python3
"""
Build a manifest for the Pangen dataset that links every image to the
...
def collect_paths() -> list[str]:
    """
    Return paths for all .nii.gz files in the Pangen dataset.
    """
    # The files are directly in this directory
    root_dir = pathlib.Path("/mnt/c/Projects/thesis_project/Data/PanGen")
    ...
    paths = []
    for pattern in ["sub-P*_T1w.nii.gz", "sub-P*_run-*_T1w.nii.gz", "sub-P*_acq-*_T1w.nii.gz"]:
...
def build_manifest(paths: list[str]) -> pd.DataFrame:
    ...
    participants_tsv = "/mnt/c/Projects/thesis_project/brain_age_pred/data/dataset_configuration/all_demographics/pangen_participants.tsv"
    ...
    df_participants_clean = df_participants[participant_columns].dropna()
...
def main():
    ...
    output_file = script_dir / "image_manifest_pangen.tsv"
    ...
    data_dir = pathlib.Path("/mnt/c/Projects/thesis_project/Data/PanGen")
    data_output = data_dir / "image_manifest_pangen.tsv"
