#!/usr/bin/env python3
"""
Build a manifest for the CoRR dataset that links every image to the
...
def extract_subject_id(path: str) -> str:
    ...
    # Remove leading zeros for matching with CSV data
    subject_id_numeric = str(int(subject_id))
    
    return subject_id_numeric


# ---------------------------------------------------------------------
def collect_paths() -> list[str]:
    """
    Return paths for all .nii.gz files in the CoRR dataset.
    """
    # The files are in the CoRR directory
    root_dir = pathlib.Path("/mnt/c/Projects/thesis_project/Data/CoRR")
    ...
    paths = []
    for pattern in ["sub-*.nii.gz", "**/sub-*.nii.gz"]:
...
def build_manifest(paths: list[str]) -> pd.DataFrame:
    ...
    """
    # Load CoRR aggregated phenotypic data
    corr_csv = "/mnt/c/Projects/thesis_project/brain_age_pred/data/dataset_configuration/all_demographics/CoRR_AggregatedPhenotypicData.csv"
    df_corr = pd.read_csv(corr_csv, dtype=str)
    ...
    df_corr_clean = df_corr.rename(columns={
        'SUBID': 'participant_id',
        'AGE_AT_SCAN_1': 'age',
        'SEX': 'sex'
    })
    
    # Select columns to include from CoRR data
    participant_columns = ["participant_id", "age", "sex"]
    
    # Filter out rows with missing age or sex data
    df_corr_clean = df_corr_clean[participant_columns].dropna()
    
    # Ensure each subject appears only once by taking the first session
    df_corr_clean = df_corr_clean.drop_duplicates(subset=['participant_id'], keep='first')
    
    # Convert participant_id to string for merging
    df_corr_clean['participant_id'] = df_corr_clean['participant_id'].astype(str)
...
def main():
    ...
    output_file = script_dir / "image_manifest_corr.tsv"
    ...
    data_dir = pathlib.Path("/mnt/c/Projects/thesis_project/Data/CoRR")
    data_output = data_dir / "image_manifest_corr.tsv"
